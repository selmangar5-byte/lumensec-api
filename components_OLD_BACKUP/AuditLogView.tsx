...
import LogInvestigationModal from './LogInvestigationModal';
        ...
        <div className="p-6 border-b border-slate-800">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-sm font-bold text-white uppercase tracking-wider">{t.auditLogExplorer}</h3>
            </div>
          ...
          <div className="flex gap-2">
            {(['ALL', 'SUCCESS', 'WARNING', 'ERROR'] as const).map((status) => (
              ...
              >
                {t[status.toLowerCase() as keyof typeof t]}
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="overflow-y-auto max-h-[400px]">
          <table className="w-full">
            <thead className="sticky top-0 bg-slate-900 border-b border-slate-800">
              <tr>
                <th className="text-left text-[10px] text-slate-500 font-mono uppercase tracking-wider px-4 py-3">{t.timestamp}</th>
                <th className="text-left text-[10px] text-slate-500 font-mono uppercase tracking-wider px-4 py-3">{t.user}</th>
                <th className="text-left text-[10px] text-slate-500 font-mono uppercase tracking-wider px-4 py-3">{t.action}</th>
                <th className="text-left text-[10px] text-slate-500 font-mono uppercase tracking-wider px-4 py-3">{t.resource}</th>
                <th className="text-left text-[10px] text-slate-500 font-mono uppercase tracking-wider px-4 py-3">{t.status}</th>
              </tr>
            </thead>
            <tbody>
        ...
        <div className="p-4 bg-slate-900/50 border-t border-slate-800">
          <div className="flex items-center justify-between">
            <p className="text-[10px] text-slate-500 font-mono">
              {t.lastUpdate}: {new Date().toLocaleTimeString('fr-FR')}
            </p>
            <p className="text-[10px] text-slate-500 font-mono">
              {t.displaying}: {filteredLogs.length}/{mockAuditLogs.length} {t.events.toLowerCase()}
            </p>
          </div>
        </div>
      </div>
